...
const QuestionCard: React.FC<QuestionCardProps> = ({ question, categoryId }) => {
  ...
  const difficultyColor = {
    ...
    Medium: 'text-yellow-600 dark:text-yellow-400 border-yellow-600 dark:border-yellow-400 bg-yellow-50 dark:bg-yellow-900/20',
      ...
      <div className="p-5">
        <div className="flex justify-between items-start mb-2">
          <h3 className="text-lg font-semibold flex items-center">
            {solved && <CheckCircle2 className="h-4 w-4 text-green-500 mr-2 flex-shrink-0" />}
            {question.title}
          </h3>
          <span className={`text-xs px-2 py-1 rounded border ${difficultyColor}`}>
            {question.difficulty}
          </span>
        </div>
