...
const FilterBar: React.FC<FilterBarProps> = ({ categoryId, totalQuestions }) => {
  const { filter, setFilter } = useFilter();
  const { getSolvedCount } = useProgress();
  const solvedCount = getSolvedCount(categoryId);

  return (
    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white dark:bg-gray-800 rounded-lg shadow-sm p-4 mb-6 border border-gray-200 dark:border-gray-700">
      ...
      <div className="flex items-center text-sm text-gray-600 dark:text-gray-400">
        <span className="font-medium text-indigo-600 dark:text-indigo-400">{solvedCount}</span>
